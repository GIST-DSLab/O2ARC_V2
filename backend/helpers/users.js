...
var getSuccessList = async function(userName){
  const query = 'SELECT task_id, count(DISTINCT subtask_id) AS solcnt, max(subtask_count) AS totalcnt FROM submission WHERE user_name = ? AND success = 1 GROUP BY task_id';
  return new Promise((resolve, reject) => {
