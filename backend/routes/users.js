...
router.get("/:id/miniarcs", async function (req, res, next) {
	...
	// let result = await userhelper.fetchSuccessProblem(userName);
...
router.get("/eval", async (req, res, next) => {
	...
	// res.send(result);
	// Instead of sending the result, render the eval view and pass the data to it
	res.render("eval", { data: result });
});
